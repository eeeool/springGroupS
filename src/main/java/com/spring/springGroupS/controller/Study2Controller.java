...
//		((int)(Math.random()*(최대값-최소값+1)) + 최소값;
	...
	@ResponseBody
	@PostMapping("/random/AlphaNumbericCheck")
	public String AlphaNumbericCheckPost() {
	...
	public String validatorFormPost(@Validated TransactionVO vo, BindingResult br) {
		if (br.hasFieldErrors()) {
			...
			int res = study2Service.setValidatorFormOk(vo);
			return "회원가입 완료";
		}		
	}
...
//Kakaomap(클릭한위치에마커표시 DB저정하기)
	...
	public int kakaoAddressDeletePost(String address) {
		return study2Service.kakaoAddressDeletePost(address);				
	}
...
//Kakaomap(DB에 저장된 장소 표시/이동하기)
  ...
  // 썸네일 이미지 삭제처리(전체파일삭제)
	...
	public ArrayList<CrawlingVO> jsoup2Post() throws IOException {
		...
		selects = document.select("div.cnf_journal_name");
		for (Element select : selects) {
		...
		for (int i=0; i<imageVos.size(); i++) {
			...
			vo.setItem1(imageVos.get(i));
			vo.setItem1(journalVos.get(i));
			System.out.println("vo: " + vo);
			vos.add(vo);
		}

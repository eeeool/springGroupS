...
<head>
    ...
    /* 별 위에 마우스 올리면 노랑색으로 출력 */
    #starForm label:hover {
      text-shadow: 0 0 0 rgba(250, 200, 0, 0.98);
    }
    
    /* 형제선택자(~)을 이용하여 hover시 같은 형제의 label은 같이 노랑색으로 처리 */
    #starForm label:hover ~ label {
      text-shadow: 0 0 0 rgba(250, 200, 0, 0.98);
    }
    
    /* 라디오 버튼을 선택하면(checked), 해당 별과 그 앞의 별들을 노랑색으로 유지시킴 */
...
<div class="container">
	...
	<table class="table table-bordered text-center">
		...
		<tr>
			...
			<td>
				...
				<c:forEach var="fName" items="${fNames}" varStatus="st">
					<a href="${ctp}/pds/${fSNames[st.index]}" onclick="downNumCheck(${vo.idx})" download="${fName}">${fName}</a><br/>
				</c:forEach>
				(<fmt:formatNumber value="${vo.FSize/1024}" pattern="#,##0" />Byte)
			</td>
			<th>다운횟수</th>
			<td>${vo.downNum}</td>
		</tr>
		...
		<tr>
			<th>상세내역</th>
			<td colspan="3" class="text-start">${fn:replace(vo.content, LF, "<br/>")}</td>
		</tr>
	</table>
	<div class="row">

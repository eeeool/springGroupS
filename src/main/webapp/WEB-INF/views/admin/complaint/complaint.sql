/*
 	신고 처리
 	complaintSw: 
 		H(감추기-board 테이블의 complaint 필드값을 'HI', complaint 테이블에서 progress 필드값을 처리중('H'))
 		S(보이기-신고해제(?) 해제시는 board 테이블의 complaint 필드값을 'NO')
 		D(삭제하기-board 테이블의 해당글을 삭제처리, complaint 테이블에서 progress 필드값을 처리중('D'))
 */
